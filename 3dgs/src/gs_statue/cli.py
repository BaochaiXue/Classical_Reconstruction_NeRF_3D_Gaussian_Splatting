"""Command-line entry point for the statue pipeline."""
...
from .pipeline.diagnostics import load_camera_centers, plot_camera_poses
from .pipeline.export_assets import export_cameras, export_gaussian_splat
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Statue video → Nerfstudio 3DGS pipeline orchestrator."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def main(argv: list[str] | None = None) -> None:
    ...
    splats_dir = Path("exports/splats") / scene_name
    ...
    if exports_cfg.get("cameras", True):
        ...
        if exports_cfg.get("figures", True):
            centers = load_camera_centers(cameras_dir)
            plot_camera_poses(centers, figures_dir / "camera_path.png")

    splat_cfg = exports_cfg.get("splats", True)
    splat_enabled = True
    splat_args = None
    if isinstance(splat_cfg, dict):
        splat_enabled = splat_cfg.get("enabled", True)
        splat_args = splat_cfg.get("extra_args")
    else:
        splat_enabled = bool(splat_cfg)
    if splat_enabled:
        export_gaussian_splat(config_path, splats_dir, extra_args=splat_args)
    ...
    if render_cfg.get("make_orbit_video", False):
        ...
        if camera_path.exists():
            ...
            print(
                f"[warn] Camera path {camera_path} not found; "
                "skipping orbit render."
            )

    if render_cfg.get("make_interpolate_video", False):
        interpolate_name = render_cfg.get("interpolate_video_name", "interpolate.mp4")
    render_interpolate_video(
        config_path,
        videos_dir / f"{scene_name}_{interpolate_name}",
    )
